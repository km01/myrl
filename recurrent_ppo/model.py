...
class ActorCritic(nn.Module):
    ...
    def forward(self, s):
        ...
        policy_param = self.actor(s)
        val_pred = self.critic(s)
        return policy_param, val_pred

    @torch.no_grad()
    def step(self, s, det=False):
        s = torch.FloatTensor(s).to(self.device())
        policy_param, val_pred = self(s)

        policy = self.policy_class(policy_param)
        act = policy.sample(det)
        log_prob = policy.log_prob(act)
        env_act = policy.as_env(act)

        agent_info = {'act': act.detach().cpu().numpy(),
